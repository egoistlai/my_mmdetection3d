...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    MIN_HEIGHT = [40, 25, 25]  # 假设 Truck 和 Car/Cyclist 使用相似的高度阈值
    MAX_OCCLUSION = [0, 1, 2]
...
def kitti_eval(gt_annos,
    ...
    overlap_0_7 = np.array([[0.7, 0.5, 0.7], 
                            [0.7, 0.5, 0.7],
                            [0.7, 0.5, 0.7]])
    overlap_0_5 = np.array([[0.7, 0.5, 0.7], 
                            [0.5, 0.25, 0.5],
                            [0.5, 0.25, 0.5]])
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # 新形状: [2, 3, 3]
...
def kitti_eval_coco_style(gt_annos, dt_annos, current_classes):
    ...
    class_to_range = {
        0: [0.5, 0.95, 10],  # Car
        1: [0.25, 0.7, 10],   # Cyclist
        2: [0.5, 0.95, 10],  # Truck (使用和Car相同的范围)
    }
    name_to_class = {v: n for n, v in class_to_name.items()}
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
