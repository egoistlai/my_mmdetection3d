...
#         """从文件中加载点云数据"""
#         points = np.fromfile(pts_filename, dtype=np.float32)
#         points = points.reshape(-1, 8)  # 重塑为 [N, 8]
#         return points
...
class LoadPointsFromFile(BaseTransform):
    ...
    Args:
        ...
        use_dim (list[int] | int): Which dimensions of the points to use.
            Defaults to [0, 1, 2, 3, 4]. For DRAD dataset, we use [x, y, z, doppler, power].
        shift_height (bool): Whether to use shifted height. Defaults to False.
        use_color (bool): Whether to use color features. Defaults to False.
        norm_intensity (bool): Whether to normlize the intensity. Defaults to
            False.
    ...
    def __init__(self,
                 coord_type: str,
                 load_dim: int = 8,  # 修改为8维
                 use_dim: Union[int, List[int]] = [0, 1, 2, 3, 4],  # 默认使用x,y,z,多普勒,功率
                 shift_height: bool = False,
                 use_color: bool = False,
                 norm_intensity: bool = False,
                 norm_elongation: bool = False,
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.norm_intensity:
            ...
            # 归一化多普勒速度 (第3维)
            points[:, 3] = np.tanh(points[:, 3] / 10.0)  # 假设多普勒速度范围在±10m/s
        
        if self.norm_elongation:
            assert len(self.use_dim) >= 5, \
                f'When using elongation norm, expect used dimensions >= 5, got {len(self.use_dim)}'
            # 归一化功率 (第4维)
            points[:, 4] = np.tanh(points[:, 4] / 10.0)  # 假设功率范围在±10dB
